...
export default class User {
  ...
  async validate() {
    ...
    try {
      ...
      let res = await request.get('https://github.com/' + user.login).promise();
      return !res.notFound;
    } catch (e) {
      return false;
    }
  ...
  static async fromLogin(login) {
    let model = await users.findOneAsync({
      login: login
    });
  ...
  static async fromToken(token, ref) {
    ...
    const gh = await P.promisify(github.user.get, github.user)({});
