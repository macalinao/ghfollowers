    ...
    return new P(function(resolve) {
      ...
      async.eachSeries(docs, function(otherModel, done) {
        ...
        return otherUser.follow(thiz.login).then(function(res) {
          ...
          thiz.invalidate().then(done, done);
        });
...
User.prototype.updateFollowedBy = function() {
  var me = this.login;
  var remove = [];
  return P.all(this.model.followedBy, function(login) {
...
/**
 * Invalidates this user by removing their followers and reducing their cap.
 */
User.prototype.invalidate = function() {
  this.getFollowing().then(function(users) {
    return P.map(users, function(model) {
      return fromModel(model).updateFollowedBy();
    });
  }).then(function() {
    return this.removeFollowers();
    // TODO reduce cap
  });
};
